...
ローカルサーバーが必要になるのは、**自分で用意したファイルをモジュールとして読み込む**ときです。この教材の範囲では出てきませんが、先に進むと必ず出会います。

- JavaScript を別ファイルに分けて `<script type="module" src="app.js">` と書いた場合
- `import './todo.js'` のように自分で書いたファイルを読み込んだ場合
- `fetch()` でローカルのファイルを読み込んだ場合

`type="module"` のスクリプトは CORS のルールで読み込まれ、`file://` のページは「オリジンなし(null)」として扱われるため、同じフォルダにあるファイルでも拒否されます。一方、CDN(https)からの読み込みは `Access-Control-Allow-Origin: *` が返るので通ります。だからこの教材は `file://` でも動きます。
